...
Build recomendado desde la raiz del proyecto:

```bash
docker build -f infra/docker/Dockerfile -t tu-espacio .
```
...
- El asistente usa reglas locales y datos existentes del sistema en lugar de proveedores IA externos.
- React se organiza en `components/`, `pages/` y `services/` para separar UI, navegacion y llamadas API.
- Los archivos de soporte se agrupan en `docs/`, `config/`, `infra/` y `logs/` para mantener la raiz operacional limpia.
- La raiz del proyecto usa una politica de allowlist definida en `config/tooling/project-layout.json`.
- Los scripts operativos viven en `../scripts/` a nivel workspace para evitar duplicacion y mantener una sola capa de mantenimiento.
...
- Validar layout del repo: `python ../scripts/quality/validate_layout.py`
