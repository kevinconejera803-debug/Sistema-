...
from app import app, _validate_iso_date
from utils import (
    validate_event_data, validate_contact_data, validate_assignment_data,
    validate_iso_date, build_safe_update
)
...
class TestBuildSafeUpdate:
    """Tests de construcción segura de SQL UPDATE."""
    
    def test_build_safe_update_valid(self):
        """Construye SQL UPDATE válido."""
        sql, vals = build_safe_update(
            "events",
            {"title": "Nuevo título", "notes": "Nuevas notas"},
            5,
            {"title", "notes", "start_iso"}
        )
        assert sql is not None
        assert "UPDATE events" in sql
        assert "title = ?" in sql
        assert "notes = ?" in sql
        assert vals[-1] == 5  # ID es último
    
    def test_build_safe_update_empty(self):
        """Retorna None si sin cambios."""
        sql, vals = build_safe_update(
            "events",
            {},
            5,
    ...
    def test_build_safe_update_filters_invalid_fields(self):
        """Filtra campos no permitidos."""
        sql, vals = build_safe_update(
            "events",
            {"title": "Test", "id": 999, "hack": "value"},  # id y hack no permitidos
            5,
            {"title"}  # Solo title permitido
        )
        assert "id" not in sql.lower() or "id = ?" not in sql
        assert "hack" not in sql
...
class TestRoutes:
    ...
    def test_api_mercados(self, client):
